...
image_folder_path = '../data/224x224_images'
data_entry_path = '../data/Data_Entry_2017_v2020.csv'
bbox_list_path = '../data/BBox_List_2017.csv'
train_txt_path = '../data/default_split/train_split.txt'
valid_txt_path = '../data/default_split/val_split.txt'
data_path = './output' # ouput folder for preprocessed data
test_txt_path = "../data/default_split/test_split.txt" #images with bounding boxes
...
#     if i % 1000==0:
#         print(i)
X = np.array([np.array(cv2.imread(image_folder_path + img, cv2.IMREAD_GRAYSCALE)) for img in train_list])
train_X.append(X)
...
X = np.array([np.array(cv2.imread(image_folder_path + img, cv2.IMREAD_GRAYSCALE)) for img in valid_list])
valid_X.append(X)
...
X = np.array([np.array(cv2.imread(image_folder_path + img, cv2.IMREAD_GRAYSCALE)) for img in test_list])
test_X.append(X)

test_X = np.array(test_X)
np.save("test_X_small.npy", test_X)
